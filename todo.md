...
## 工作流程节点进度体验（本轮）
- [x] 为每个流程节点添加细长阶段进度条与百分比读数
- [x] 使用当前步骤、总体进度和任务状态计算四阶段的细粒度执行进度
- [x] 为运行中节点增加 reduced-motion 兼容的低干扰呼吸灯效果
- [x] 验证运行态、完成态、桌面/手机端以及 TypeScript、Vitest、生产构建（已实测等待态显示四个 0% 进度条；运行时完成节点为 100%、当前候选构建节点跟随真实总体进度、后续节点为 0%；任务完成后四节点均为 100%）
- [x] 保存节点进度与呼吸灯体验检查点

## 工作流控制与完成通知（本轮）
- [x] 在左侧工作流面板新增醒目的暂停/恢复与取消任务控制入口
- [x] 将工作流控制与现有 tRPC 暂停、恢复、取消操作联动并反馈错误状态
- [x] 在任务首次完成时显示全局成功通知，并提供查看最终候选结果快捷入口
- [x] 验证暂停、恢复、取消、完成通知与结果跳转的桌面/手机表现，以及 TypeScript、Vitest、生产构建（已实测运行态显示左侧暂停/取消；点击暂停后工作流切换为“恢复/取消”，当前节点显示暂停状态；取消后任务进入已停止/失败状态并移除控制条，且可新建后再次启动；完成时成功通知显示候选数量与“查看最终结果”，点击后平滑定位候选列表；修复处理器引用后新会话空态与输入过程未再触发更新循环）
- [x] 保存工作流控制与完成通知检查点
