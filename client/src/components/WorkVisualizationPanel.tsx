...
import { getWorkflowNodeStates, type WorkflowNodeStatus } from '@shared/workflowNodeStatus';
...
const pipelineNodes = [
  ...
  { icon: Dna, label: '候选构建', summary: '生成并优化序列', top: 'top-[52px]', description: '生成多样化候选多肽，并通过序列级优化扩大可探索的设计空间。' },
  { icon: FlaskConical, label: '性质筛选', summary: '评估理化性质', top: 'top-[104px]', description: '计算分子量、电荷、疏水性、稳定性和 pI 等指标，过滤不满足设计约束的候选。' },
  { icon: Trophy, label: '亲和排序', summary: '输出优先级候选', top: 'top-[156px]', description: '整合序列与亲和力评分，对保留候选进行排序并输出高优先级多肽结果。' },
];
  ...
  return (
    <div className="relative flex h-full flex-col items-center justify-center gap-5 overflow-hidden px-6">
      ...
      <div className="relative h-[210px] w-[260px]" aria-label="多肽自主设计工作流程：需求解析、候选构建、性质筛选、亲和排序">
        <motion.div className="absolute inset-x-0 top-1/2 h-[190px] -translate-y-1/2 rounded-2xl border border-primary/30 bg-gradient-to-b from-primary/10 via-card/90 to-accent/10 shadow-[0_18px_58px_-30px_rgb(16_185_129_/_0.7)] dark:border-primary/40 dark:from-primary/15 dark:via-card/95 dark:to-accent/15" animate={hoveredNode === null ? { opacity: 1 } : { opacity: 1, scale: 1.015 }} transition={{ duration: shouldReduceMotion ? 0 : 0.2, ease: [0.23, 1, 0.32, 1] }} />
        ...
        {progress.status === 'running' && <motion.span className="absolute left-1/2 top-[19px] h-2 w-2 -translate-x-1/2 rounded-full bg-primary shadow-[0_0_12px_rgba(16,185,129,0.9)]" animate={shouldReduceMotion ? undefined : { y: [0, 160, 0], opacity: [0.35, 1, 0.35] }} transition={{ duration: 4.2, repeat: Infinity, ease: 'easeInOut' }} />}
            ...
            <Tooltip key={label}>
              <TooltipTrigger asChild>
                <motion.div
                  ...
                  className={`absolute left-1/2 ${top} z-10 flex w-[214px] -translate-x-1/2 items-center gap-3 rounded-xl border px-3 py-2 outline-none backdrop-blur-sm transition-colors focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-background ${meta.card}`}
                  animate={isHovered ? { scale: 1.035, x: 3 } : { scale: 1, x: 0 }}
                  transition={{ duration: shouldReduceMotion ? 0 : 0.18, ease: [0.23, 1, 0.32, 1] }}
                >
                  <span className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-lg ${meta.iconTone}`}><Icon className="h-3.5 w-3.5" /></span>
                  <span className="min-w-0 flex-1 text-left"><span className="block text-[9px] font-medium uppercase tracking-[0.13em] text-primary/85">Step 0{index + 1} · {summary}</span><span className="block text-xs font-semibold text-foreground">{label}</span></span>
                  <span className={`flex shrink-0 items-center gap-1 text-[9px] font-semibold ${meta.labelTone}`}><StatusIcon className={`h-3 w-3 ${nodeState === 'running' && !shouldReduceMotion ? 'animate-spin' : ''}`} />{meta.label}</span>
                </motion.div>
              </TooltipTrigger>
              <TooltipContent side="right" sideOffset={12} className="z-[60] max-w-64 border-primary/65 bg-popover text-popover-foreground shadow-2xl shadow-black/60 ring-primary/30 dark:border-primary/70 dark:bg-popover dark:text-popover-foreground dark:shadow-black/75 dark:ring-primary/40">
                <p className="flex items-center gap-1.5 font-bold tracking-tight text-popover-foreground"><span className="rounded bg-primary/20 px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-[0.12em] text-primary">Step 0{index + 1}</span>{label}<span className={`ml-auto text-[9px] ${meta.labelTone}`}>{meta.label}</span></p>
